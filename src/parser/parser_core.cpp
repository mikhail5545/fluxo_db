...
int Parser::get_precedence(const TokenType type) {
    switch (type) {
...
BinaryOp::Op Parser::token_to_binop(const TokenType type) {
    switch (type) {
        case TokenType::PLUS:
...
std::string Parser::errMsg(const Token& token, const std::string& msg) {
    return msg + " at line " + std::to_string(token.line) + ", column " + std::to_string(token.column);
}

DataType Parser::token_to_data_type(const Token& token) {
